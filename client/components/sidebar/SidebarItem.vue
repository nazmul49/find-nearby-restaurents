<template>
  <div class="flex flex-col">
    <template v-for="(l, i) in links" :key="i">
      <NuxtLink v-if="!l.items" :to="l.route" class="inline-flex items-center gap-4 p-3 px-4 text-left text-[15px]">
      ...
      <HDisclosure as="div" class="w-full" v-slot="{ open }" v-else>
        <HDisclosureButton class="inline-flex w-full items-center justify-between p-3 px-4 text-left text-[15px]">
        ...
        <TransitionExpand>
          <HDisclosurePanel class="mx-6 flex flex-col border-l px-3">
            <template v-for="(i, j) in l.items" :key="j">
              <NuxtLink class="rounded-md px-3 py-1.5 text-sm hover:bg-muted" :to="i.route">
                {{ i.title }}
              </NuxtLink>
            </template>
          </HDisclosurePanel>
